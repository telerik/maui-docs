...
Before you can add Telerik Theming to your app, make sure that it is configured to use the Telerik UI for .NET MAUI components. For more details, see the [Getting Started with Telerik UI for .NET MAUI in Visual Studio]({%slug maui-quick-start %}#set-up-telerik-development-environment) or [Visual Studio Code]({%slug maui-quick-start %}#set-up-telerik-development-environment) articles.
