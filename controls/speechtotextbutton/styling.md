...
* (Desktop) `MouseOver`&mdash;The state when the mouse cursor is hovering over the button.
...
* (Desktop) `ListeningMouseOver`&mdash;The state when the mouse cursor is hovering over the button when in `Listening` state.
