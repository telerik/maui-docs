...
* `ItemHold`&mdash;Raised when the user performs a hold gesture on a collection view item. The `ItemHold` event handler receives two parameters:
	* The `sender` argument, which is of type `object`, but can be cast to the `RadCollectionView` type.
	* A `CollectionViewItemHoldEventArgs` object, which has a reference to the held item through its `Item`(`object`) property.
...
![Telerik UI for .NET MAUI CollectionView GroupTapped event executes when tapping on a group, so collapsing or expanding the group depends on the current state](images/collectionview-group-tapped.gif "Telerik .NET MAUI CollectionView")
...
![Telerik UI for .NET MAUI CollectionView ItemTapped event executes when tapping on an item](images/collectionview-item-tapped.gif "Telerik .NET MAUI CollectionView")
