...
| `Space` | Selects the currently focused item in `Single` `SelectionMode`. When `SelectionMode` is `SingleDeselect`, the `Space` key selects/deselects the current item. When `SelectionMode` is multiple, the `Space` key selects/deselects the current item. |
