...
* `SuggestionViewMaxHeight` (`double`)&mdash;Defines the max height of the `SuggestionView`. It is recommended to use `SuggestionViewMaxHeight`. If you want to use both, the `SuggestionViewHeight` value must be set to lower value than the `SuggestionViewMaxHeight`.
