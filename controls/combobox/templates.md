...
* `SelectionBoxTemplate`(`DataTemplate`)&mdash;Defines the template of the selected item in the box section of the control. This property applies when the `RadComboBox` is non-editable (`IsEditable` set to `false`) and `SelectionMode` is `Single`.
