...
* `ThumbStyle` (`Style` with target type `Telerik.Maui.Controls.RangeSlider.SliderThumb`)&mdash;Defines a custom style for the Slider's thumb.
...
* `RangeTrackStyle` (`Style` with target type `Telerik.Maui.Controls.RangeSlider.SliderRangeTrack`)&mdash;Defines a custom style for the Slider's range track.
