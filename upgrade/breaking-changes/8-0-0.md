...
* The `DataGridToggleRowDetailsColumn`'`CellContentStyle` type is changed from `DataGridToggleRowDetailsCellStyle` to `Style` and targets the `DataGridToggleRowDetailsCellAppearance` class. For more details on how to style the column footer, review the [Columns Styling]({%slug datagrid-columns-styling%}) article.
